...
This master document provides the high-level architecture and decision rationale. Detailed implementation guides are in separate documents:

### Core Documents
- **[SETUP_GUIDE.md](#)** - Step-by-step environment setup (Weekend Day 1 Morning)
- **[AESTHETIC_SPEC.md](#)** - Visual design, frame mockups, prompt engineering
- **[BACKEND_ARCHITECTURE.md](#)** - Python system design and implementation
- **[COMFYUI_WORKFLOW.md](#)** - Generation pipeline and Flux configuration
- **[RAINMETER_WIDGET.md](#)** - Frontend display system
- **[WEEKEND_SPRINT.md](#)** - Hour-by-hour implementation timeline

### Reference Documents
- **[TROUBLESHOOTING.md](#)** - Common issues and solutions
- **[OPTIMIZATION_GUIDE.md](#)** - Performance tuning post-MVP
- **[FUTURE_ROADMAP.md](#)** - Post-weekend enhancements
...
│   │
│   ├── core/                       # Main generation logic
│   │   ├── __init__.py
│   │   ├── controller.py           # Main loop orchestration
│   │   ├── comfyui_api.py          # API wrapper
│   │   └── generator.py            # Mode selection and execution
│   │
...
│   ├── interpolation/              # Latent space operations
│   │   ├── __init__.py
│   │   ├── spherical_lerp.py       # Interpolation algorithms
│   │   └── latent_encoder.py       # VAE encoding/decoding
│   │
│   ├── utils/                      # Helpers
│   │   ├── __init__.py
│   │   ├── file_ops.py             # Atomic writes, file management
│   │   ├── logging_config.py       # Logging setup
│   │   └── system_monitor.py       # Game detection, GPU monitoring
│   │
│   └── requirements.txt            # Python dependencies
│
├── comfyui_workflows/              # ComfyUI workflow JSONs
│   ├── flux_img2img.json           # Basic img2img workflow
│   ├── flux_interpolation.json     # Latent interpolation workflow
│   └── flux_hybrid.json            # Combined workflow
│
├── seeds/                          # Base images (your angels)
│   ├── angels/
│   │   ├── bg_5.png
│   │   ├── num_1.png
│   │   ├── num_3.png
│   │   └── [more images]
│   │
│   ├── metadata.json               # Image tags, embeddings
│   └── README.md                   # How to add new seeds
│
├── cache/                          # Generated image cache
│   ├── images/                     # Cached PNG files
│   └── metadata/                   # CLIP embeddings, generation params
│
├── output/                         # Live output buffer (Rainmeter reads)
│   ├── current_frame.png           # Active display
│   ├── next_frame.png              # Pre-generated (ready to swap)
│   ├── previous_frame.png          # For crossfade
│   └── status.json                 # Generation status metadata
│
├── rainmeter/                      # Rainmeter widget
│   └── DreamWindow/
│       ├── DreamWindow.ini         # Main widget skin
│       ├── Variables.inc           # User-editable settings
│       │
│       └── @Resources/
│           ├── Images/
│           │   ├── border_frame.png      # Main border
│           │   ├── corner_accent.png     # Corner brackets
│           │   ├── glow_overlay.png      # Generation indicator
│           │   └── glitch_overlay.png    # Optional glitch effect
│           │
│           ├── Fonts/
│           │   └── MonaspaceNeon.ttf     # Monospace font (optional)
│           │
│           └── Scripts/
│               └── CrossfadeHandler.lua  # Advanced crossfade (if needed)
│
...
```python
def atomic_write(image, output_path):
    ...
    # Atomic operation on Windows
    shutil.move(tmp_path, output_path)
```

**Frame Swap Protocol:**
1. Generate new frame
2. Write to `next_frame.png` (atomic)
3. Copy `current_frame.png` → `previous_frame.png`
4. Move `next_frame.png` → `current_frame.png` (atomic)
5. Rainmeter detects change, triggers crossfade
...
**If you're ready to start building RIGHT NOW:**
→ Go to **SETUP_GUIDE.md** and begin the Saturday Morning sprint

**If you want to see the design first:**
→ Go to **AESTHETIC_SPEC.md** for frame mockups and visual specs

**If you want to understand the code architecture:**
→ Go to **BACKEND_ARCHITECTURE.md** for detailed system design

**If you want the detailed timeline:**
→ Go to **WEEKEND_SPRINT.md** for hour-by-hour breakdown
...
MASTER_PLAN.md (you are here)
    ...
    │       └─► (Code implementation)
    │
    ├─► WEEKEND_SPRINT.md ────────► (Your timeline)
    │
...
- **v2.0** (2025-11-07): Complete rewrite with Flux.1, modular architecture, comprehensive docs
...
The next document you should read is **SETUP_GUIDE.md** - that's where we start getting our hands dirty.
