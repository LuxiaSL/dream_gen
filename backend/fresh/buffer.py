...
class FreshFrameBuffer:
    ...
    async def ensure_ready(self) -> bool:
        """
        DEPRECATED: Use populate_all() at startup instead.
        
        Kept for backwards compatibility but does nothing in new design.
        """
        logger.warning(
            "[FRESH] ensure_ready() is deprecated. "
            "Use populate_all() at startup instead."
        )
        return self._startup_complete
    
    def consume(self) -> Optional[BufferedFrame]:
        """
        DEPRECATED: Use select_and_consume() instead.
        
        This sync method cannot properly handle the new per-template design.
        """
        logger.warning(
            "[FRESH] consume() is deprecated. "
            "Use await select_and_consume() instead."
        )
        # Return first available frame for backwards compat
        for template_id, frame in self._buffer.items():
...
async def test_fresh_frame_buffer() -> bool:
    ...
    """
    print("=" * 60)
    print("Testing FreshFrameBuffer (per-template mode)...")
    print("=" * 60)
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            # Create test images for mock generation
            ...
            # Create mock prompt system with multiple templates
            mock_prompt = MagicMock()
            mock_prompt.get_available_templates.return_value = ["template_a", "template_b", "template_c"]
            
            # Create mock templates
            mock_template = MagicMock()
            mock_template.structure = "{adjective} {noun}"
            mock_template.slots = [("adjective", 1, " "), ("noun", 1, " ")]
            
            mock_prompt.templates = {
                "template_a": mock_template,
                "template_b": mock_template,
                "template_c": mock_template
            }
            
            # Create mock components
            mock_comp = MagicMock()
            mock_comp.word = "ethereal"
            mock_comp.opposite = "harsh"
            ...
            assert not buffer._startup_complete, "Should not be complete before populate"
            assert not buffer.is_ready(), "Buffer should start empty"
            print("✓ Initial state correct")
            
            print("\n3. Testing populate_all()...")
            await buffer.populate_all()
            assert buffer._startup_complete, "Should be complete after populate"
            assert buffer.is_fully_populated(), "All templates should have frames"
            assert buffer.get_ready_count() == 3, f"Should have 3 frames, got {buffer.get_ready_count()}"
            print("✓ populate_all() works")
            
            print("\n4. Testing select_and_consume()...")
            frame1 = await buffer.select_and_consume()
            assert frame1 is not None, "Should get a frame"
            assert frame1.template_id in ["template_a", "template_b", "template_c"]
            print(f"✓ Got frame for '{frame1.template_id}'")
            
            print("\n5. Testing selection pool tracking...")
            assert len(buffer._available_pool) == 2, f"Pool should have 2 remaining, got {len(buffer._available_pool)}"
            assert len(buffer._used_this_cycle) == 1, f"Should have 1 used, got {len(buffer._used_this_cycle)}"
            print("✓ Pool tracking works")
            
            print("\n6. Consuming all frames...")
            frame2 = await buffer.select_and_consume()
            frame3 = await buffer.select_and_consume()
            assert frame2.template_id != frame1.template_id, "Should get different template"
            assert frame3.template_id not in [frame1.template_id, frame2.template_id], "Should get third template"
            print(f"✓ Got all 3 different templates: {frame1.template_id}, {frame2.template_id}, {frame3.template_id}")
            
            print("\n7. Testing pool reset...")
            assert len(buffer._available_pool) == 0, "Pool should be empty"
            # Wait a moment for regeneration tasks
            await asyncio.sleep(0.1)
            # Pool resets on next consume attempt
            print("✓ Pool tracking correct before reset")
            ...
            print(f"✓ Stats: templates={stats['total_templates']}, consumed={stats['total_consumed']}")
